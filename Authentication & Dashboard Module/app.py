...
def get_db_connection():
    # Opens a connection to the unified SQLite database
...
def update_stock(item_id):
    ...
    # 2. Connect to the unified database path you configured
    conn = get_db_connection() # Use your existing DB connection function
    
    # 3. Execute the SQL Mapping to update the inventory table
    conn.execute('UPDATE inventory SET stock = ? WHERE id = ?', 
                 (new_stock, item_id))
    
    conn.commit()
    conn.close()
    
    # 4. Redirect back to the inventory page so it refreshes with new data
